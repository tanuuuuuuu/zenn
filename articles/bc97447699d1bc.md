...
DWH（データ）とエンドユーザー（データ利用者）の間の「橋渡し」となる存在です。複雑で理解が難しいデータ構造を、ビジネスで馴染みのある用語や概念に変換することで、データサイエンティストや SQL を書ける人に頼ることなく、データにアクセスして活用できるようになります。
...
:::message
Steep を利用するためには、まず初めにセマンティックレイヤーを定義する必要があります。

自身のネイティブなセマンティックレイヤーに加えて、dbt Semantic Layer や Cube といったサードパーティ製のセマンティックレイヤーサービスを利用することもできます。
:::

Steep の最大の特徴は、無駄な機能が削ぎ落とされ、圧倒的に洗練されている UI/UX にあります。チャートの種類も、棒グラフ、折れ線グラフ、ランクチャート、ピボットテーブルなど、基本的なものに絞られています。

アナリストからするとデータの表現方法が乏しく感じるかもしれませんが、定点観測用のレポートでは基本的なチャートで十分です。Steep は、高度な分析を行うアナリスト向けというより、**データリテラシーがそこまで高くないビジネスユーザーが、日常的にメトリクスを探索・可視化するために設計された BI ツール**だと言えます。
...
---
flowchart LR
  subgraph Polaris["DWH"]
    dwh["BigQuery<br>一次情報"]
  end

  subgraph BI
    looker_studio["Looker Studio<br>二次情報"]
  end
  ...
  dwh -- 抽出 and 変形 --> power_user
  dwh -- 抽出 and 変形 ---> looker_studio["Looker Studio<br>二次情報"]
  looker_studio -- 閲覧 and 編集 --> power_user
  looker_studio -- 閲覧 --> business_user
  style dwh fill:#C8E6C9,color:#333
  style looker_studio fill:#BBDEFB,color:#333
...
- 社内トレーニングを受けたビジネスユーザー（パワーユーザー）のみが、直接 BigQuery（一次情報）を使って Looker Studio でデータを探索・分析することができた
- トレーニングを受けていないビジネスユーザーは、パワーユーザーの助けを借りて Looker Studio（二次情報）でデータを探索・分析することができた
...
- メトリクスの定義者ごとに若干の誤差（データカオス）が生じていた
- パワーユーザーの育成難易度が高かった
  - 育成に掛かっていた時間: 300〜400 時間（SQL、Looker Studio、データリテラシーなど）
  - 成功確率: 5 人に 1 人
- パワーユーザーに単純な集計依頼が集中していたため、集計を越える分析手法（予測や因果推論など）が実施できていなかった
...
flowchart LR
  ...
  dwh -- 抽出 and 変形 --> power_user
  dwh -.- steep
  steep -- 閲覧 and 編集 --> power_user & business_user
  style dwh fill:#C8E6C9,color:#333
  style steep fill:#BBDEFB,color:#333
  linkStyle 0 stroke:#00C853,fill:none
  linkStyle 2 stroke:#2962FF,fill:none
  linkStyle 1 stroke:#00C853,fill:none
  linkStyle 3 stroke:#2962FF,fill:none
...
- ビジネスユーザーが日常で扱う 80〜90% の指標が、3 時間のトレーニングを行うだけで、Steep でデータを探索・分析できるようになった
- Steep で分析するものに関しては、データカオスが解消された
- パワーユーザーは単純集計から少しずつ解放され、より本質的な業務に集中できたり、高度な分析に挑戦しやすい環境になった
