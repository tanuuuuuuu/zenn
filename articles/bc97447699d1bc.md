...
dbt Semantic Layer は、dbt で構築したデータモデル上にビジネスメトリクスの定義を一元管理する機能です。SQL を書かずに一貫性のあるメトリクスを定義でき、複数の BI ツールや分析環境から同じ定義のメトリクスを参照できます。これにより「売上の定義が部署ごとに異なる」といった問題を防ぎ、組織全体でデータドリブンな意思決定を促進します。

### Steep とは

あとで書く

Steep は、dbt Semantic Layer に特化した BI ツールです。テクニカルな知識を持たないビジネスユーザーでも、直感的な UI でメトリクスを探索・可視化できます。dbt Semantic Layer で定義されたメトリクスを自動的に読み込み、ドラッグ&ドロップ操作でダッシュボードやレポートを作成できるため、データアナリストの負担を軽減しつつ、セルフサービス分析を実現します。
